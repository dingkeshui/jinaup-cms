		...
		<div class="box" v-if="!showInfo">
			<div class="miantop">
			    <div class="maintopleft">
			    	...
			    	<el-input class="w150 mgr5" clearable size="mini" v-model="searchData.phoneNumber" placeholder="手机号码"></el-input>
				...
				<div class="tablebox">
			        <el-table
			          ...
			          <el-table-column
			            ...
			            width="100">
			            <template slot-scope="scope">
			            	<span v-if="scope.row.userLevel==0">非会员</span>
			            	<span v-if="scope.row.userLevel==1">会员</span>
			            	<span v-if="scope.row.userLevel==2">经理</span>
			            	<span v-if="scope.row.userLevel==3">总监</span>
			          ...
			          <el-table-column
			            ...
			            width="120">
			            <template slot-scope="scope">
			            	<el-tag v-if="scope.row.userType==1" size="small" type="warning">会员</el-tag>
			            	<el-tag v-if="scope.row.userType==2" size="small" type="warning">体验会员</el-tag>
			            	<el-tag v-if="scope.row.userType==0" size="small" type="success">非会员</el-tag>
			            </template>
			          </el-table-column>
			          <el-table-column
			            prop="sex"
			          ...
			          <el-table-column
			            ...
			            <template slot-scope="scope">
			            	...
			            	<div><el-button type="success" @click="changeUserLevel(scope.row)" size="mini">修改</el-button></div>
			            </template>
			          </el-table-column>
			          <el-table-column
			            label="用户来源"
...
export default {
  ...
  methods: {
  	...
  	changeUserLevel(row){
  		...
  		this.$prompt('请输入用户等级', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
        }).then(async ({ value }) => {
          this.$api.changeUserLevel({uid:row.uid,userLevel:value},data=>{
  	...
  	// 确定修改上级
  	sureChangeUserPar(){
  		this.$api.changeUserPar({uid:this.changUser.uid,parentId:this.usersitem.uid,parentName:this.usersitem.nickName},data=>{
