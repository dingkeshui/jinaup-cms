      		...
      		<el-scrollbar class="noxscroll" style="height: 100%">
		          ...
		          <div class="orderItemTop">
		            <template v-if="item.microStoreInfoVo">
		              ...
		              <span class="spanColor2 mgr10">{{item.microStoreInfoVo.storeName}}</span>
		            </template>
		            <template v-if="item.microStoreInfoVo">
		            ...
		            <el-tag v-if="item.processStatus==6" size="small" class="mgr10" type="info">已退款</el-tag>
		            <el-tag v-if="item.processStatus==9" size="small" class="mgr10" type="info">交易完成</el-tag>
		            <span class="spanColor1">用户名:</span>
		            <span class="spanColor2 mgr10" v-if="item.userInfoVo">{{item.userInfoVo.nickName}}</span>
		            <span class="spanColor1">用户ID:</span>
		            <span class="spanColor2 mgr10">{{item.buyerId}}</span>
		            <template v-if="item.userInfoVo.phoneNumber">
		            	<span class="spanColor1">用户手机:</span>
		            	<span class="spanColor2 mgr10">{{item.userInfoVo.phoneNumber}}</span>
		            </template>
		          </div>
		          <div class="orderItemMid flex">
		            <div class="flex1 orderOneItemBox">
		              <div class="pad10 orderOneItem flex">
		                <div class="flex goodsImgbox flexcenter mgr5">
		                  <img :src="item.microProductPictureVos[0].imgUrl">
		                </div>
		                <div class="flex1">
		                  <div style="min-height:40px;" class="goodsTitleBox">{{item.microProductInfoVo.title}}</div>
		                  ...
		                  <div class="fs12">
		                    ...
		                    <span class="on mgr10">{{item.microProductInfoVo.currentPrice}}</span>
		                  </div>
		                  <div v-if="item.confirmReceiptTime" class="fs12">
		                    <span class="titleSpan">收货时间:</span>
		                    <span class="on mgr10">{{$utils.getTime(item.confirmReceiptTime)}}</span>
		                  </div>
		                </div>
		              </div>
		            </div>
		            <div class="w150 flex2 br flexcenter">
		              ...
		              <div style="width:100%;" class="fs14"><span class="moneySpan">运费:</span><span>￥{{item.totalDeliveryMoney}}</span></div>
		              <div style="width:100%;" class="fs14"><span class="moneySpan">优惠券:</span><span class="red">{{item.totalDiscountAmoun?item.totalDiscountAmoun:0}}元</span></div>
		              <div style="width:100%;" class="fs14"><span class="moneySpan">红包:</span><span class="red">{{item.redEnvelopeAmount?item.redEnvelopeAmount:0}}元</span></div>
		            </div>
		            ...
		            <div class="w150 br flex2 flexcenter fs14">
		              	<div v-if="item.payStatus>1" style="width:100%;">
		              		...
		              		<span v-if="payChannel==2">支付宝支付</span>
		              		<span v-else>微信支付</span>
		          		</div>
		                <div style="width:100%;">
		                	<span class="moneySpan">发货方式:</span>
		                	<span v-if="item.receivingMethod==1">自提</span>
		                	<span v-else>快递</span>
		                </div>
		            </div>
		          ...
		          <div class="fs12 pad10 orderItemFoot">
		            ...
		            <template v-if="item.receivingMethod==2">
			            ...
			            <template v-if="item.logisticsCompany">
			            	<el-tag size="mini">{{item.logisticsCompany}}</el-tag>
    	...
    	<el-dialog
	      ...
	      <div class="box infoBox">
	      	...
	      	<div class="flex mgb5">
	      		<div class="flex1">
	      			<span class="spanColor1">下单时间</span>
	      			<span class="spanColor2">{{orderInfo.time}}</span>
	      		</div>
	      		<div class="flex1">
	      			<span class="spanColor1">订单号</span>
...
<script type="text/javascript">
	export default {
		...
		methods:{
			...
			getList(){
		    	this.$api.bOrderList(this.postData,(res)=>{
		    ...
		    downfile(){
		      ...
		      this.$api.downBOrder(downOrderData).then(data=>{
		        console.log('下载订单',data)
		        if(data){
		          if(!!window.ActiveXObject || "ActiveXObject" in window) {
		            console.log('ie浏览器')
		            let blob = new Blob([data.data])
		            let fileName = '纳小店订单.xlsx'
		            navigator.msSaveBlob(blob, fileName)
		          } else {
		            var content = data.data
		            var elink = document.createElement('a');
		            var blob = new Blob([content]);
		            elink.download = "纳小店订单.xlsx";
