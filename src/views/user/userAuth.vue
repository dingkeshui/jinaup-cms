		...
		<div class="box">
			<div class="miantop">
			    <div class="maintopleft">
					...
					<el-input class="mgr10" style="width:200px;" placeholder="用户真实姓名" v-model="searchData.trueName" size="small"></el-input>	
					<el-input class="mgr10" style="width:200px;" placeholder="身份证号" v-model="searchData.idNumber" size="small"></el-input>	
			    </div>
                  ...
                  width="150">
    		          ...
    		          label="身份证正面"
    					...
    					<template slot-scope="scope">
    						<img class="idimg" :src="scope.row.idFrontImgUrl">
    					</template>
    				  </el-table-column>
    				  <el-table-column
    		          label="身份证反面"
    					    width="170">
    					<template slot-scope="scope">
    						<img class="idimg" :src="scope.row.idReverseImgUrl">
    					</template>
    				  </el-table-column>
              <el-table-column
                  prop="refuseReason"
...
export default {
  ...
  methods: {
    ...
    pageChange(item){
  	...
  	// 获取用户认证信息列表
  	getList () {
  		console.log('获取用户认证信息列表')
  		console.log(this.postData)
  		this.$api.getUserAuthInfoList(this.postData,data=>{
  			console.log(data)
  			if(data.status==200){
